...
class Microservice {
  ...
  startDatabase (callback) {
    if (!_.isString(this.config.driver)) {
      setImmediate(callback, new Error('No databank driver configured'))
      return
    }
  ...
  static appAuthc (req, res, next) {
    const { appKeys } = req.app.config
    const cls = this || Microservice
    cls.bearerToken(req, (err, tokenString) => {
      if (err) {
  ...
  static bearerToken (req, callback) {
