...
class Microservice {
  ...
  stopNetwork() {
    if (this.srv == null) {
      this.log && this.log.debug('Skipping stopNetwork(); no server');
      return;
    }
    return new Promise((resolve, reject) => {
      const onError = err => {
  ...
  setupExpress() {
    ...
    if (this.config.metrics) {
      this.log && this.log.debug('Enabling metrics', this.config.metrics_options);
