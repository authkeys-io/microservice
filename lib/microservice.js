...
class Microservice {
  ...
  startTimingTimer() {
    return this.timingTimer = setTimeout(this.reportTiming, this.config.timingInterval);
  }
  ...
  appAuthc(req, res, next) {
    ...
    return Microservice.prototype.bearerToken(req, function(err, tokenString) {
      if (err) {
        return next(err);
  ...
  requestTimer(req, res, next) {
    ...
    res.end = (chunk, encoding) => {
      ...
      if (this.timing.count === 1) {
        ...
        return this.timing.stddev = Math.sqrt((((count - 1)*Math.pow(stddev, 2)) + (count * Math.pow(avg - this.timing.avg, 2)) + Math.pow(duration - this.timing.avg, 2))/count);
      }
    };
